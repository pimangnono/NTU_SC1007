...
int isStackPairwiseConsecutive(Stack *s)
{
    ...
    temp.ll.size = 0;
    temp.ll.head = NULL;
    int item, count, check;
    if (s->ll.size=0 || s->ll.size %2 != 0) return 0;
    else
    {
        for (count=0; count<s->ll.size; count++)
            {
                item = pop(s);
                push(&temp, item);
                check=item;
                if (check != temp.ll.head->item+1 || check != temp.ll.head->item-1) return 0;
